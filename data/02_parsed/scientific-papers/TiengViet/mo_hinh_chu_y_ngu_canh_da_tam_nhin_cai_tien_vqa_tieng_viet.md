...
ISSN: 2734-9918  
Website: https://journal.hcmue.edu.vn https://doi.org/10.54607/hcmue.js.22.2.4328(2025)
...
Tạp chí Khoa học Trường ĐHSP TPHCM — Tập 22, Số 2 (2025): 247-259
...
Mô hình chú ý ngữ cảnh đa tầm nhìn (multi-vision contextual attention model) là một phương pháp mới của Nguyen et al. (2022) đề xuất để giải quyết bài toán VQA cho tiếng Việt (gọi tắt là ViVQA), kiến trúc mô hình được minh họa qua Hình 1. Mô hình kết hợp hai phương pháp trích xuất đặc trưng hình ảnh: sử dụng ResNet để nắm bắt thông tin ngữ cảnh
...
Khối Multimodal Fusion Module chịu trách nhiệm nối và điều chỉnh đặc trưng hình ảnh và câu hỏi vào không gian chung, đảm bảo rằng thông tin từ cả hai nguồn được biểu diễn trong cùng một miền đặc trưng với cùng kích thước. Sau đó, tiếp tục xử lí biểu diễn này bằng cách áp dụng
...
$$
V_{concat\_visual} = concatenation(V_{global}, V_{local})
\tag{2}
$$

$$
V_{q,t} = concatenation[Linear(V_{concat\_visual}), Linear(V_{text})]
\tag{3}
$$

$$
V_{mha} = Self-MHA(V_{q,t}) = MHA(V_{q,t}, V_{q,t}, V_{q,t})
\tag{4}
$$
...
- $V_{concat\_visual}$ là đặc trưng đa tầm nhìn của hình ảnh.
...
$$
MHA(Q,K,V) = softmax\left(\frac{QK^T}{\sqrt{d_k}}\right)V
\tag{5}
$$
...
$$
J_{FBOAM} = Linear(V_{mha})
\tag{6}
$$

$$
J_{MVCAM} = Linear(J_{MVCAM})
\tag{7}
$$
...
$$
L_{total} = L_{cross-entropy} + L_{i,j}
\tag{9}
$$
...
$$
L_{cross-entropy} = -\sum_{i=1}^{C} y_i \log(p_i)
\tag{10}
$$
...
L_{i,j} =
-\log
\frac{\exp(\operatorname{cosine}(z_i,z_j)/T)}
{\sum_{k=1}^{2N} \mathbf{1}_{[k\ne i]}\exp(\operatorname{cosine}(z_i,z_j)/T)}
\tag{11}
$$
