...
\* Corresponding author. Email: luongthaile80@utc.edu.vn
...
Figure 2 illustrates the architecture diagram; the batch size dimension is omitted for clarity. The input sequence consists of vectors $X_1, X_2, \ldots, X_n$, each being a one-dimensional vector of length 126, corresponding to 42 hand landmarks with 3 coordinates each. These vectors are processed through two Bi-LSTM layers. The output from the last Bi-LSTM layer is a sequence of vectors $T'_1, T'_2, \ldots, T'_n$, representing the time steps. This output is then fed into an attention layer (additive or multi-head). The attention mechanism produces a single vector, which is then passed through two linear layers. Following the attention and fully-connected layers, batch normalization, ReLU activation, and dropout are applied to the output vector.
